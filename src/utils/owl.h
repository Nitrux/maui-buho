...
{
Q_NAMESPACE
enum class TABLE : uint8_t { NOTES, NOTES_SYNC, BOOKS, BOOKLETS, BOOKLETS_SYNC, NONE };

static const QMap<TABLE, QString> TABLEMAP = {{TABLE::NOTES, "notes"}, {TABLE::NOTES_SYNC, "notes_sync"}, {TABLE::BOOKS, "books"}, {TABLE::BOOKLETS, "booklets"}, {TABLE::BOOKLETS_SYNC, "booklets_sync"}};
...
static inline bool saveNoteFile(const QUrl &url, const QByteArray &data)
{
    ...
    if (file.open(QFile::WriteOnly)) {
        ...
        qWarning() << "Couldn-t open file to writte the text " << url;
